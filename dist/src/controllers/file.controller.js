import { authorize, createFolder, getFile, uploadFile, deleteFile, uploadFileImageWithBase64, } from "../ultils/uploadFile.js";
import sharp from "sharp";
import path from "path";
import fs from "fs";
import { checkBucketList, createBucket, 
// downloadFileFromBucket,
getListFile, uploadFileToBucket, uploadImageToBucket, viewFile, } from "../ultils/uploads/upload_v2.js";
import { error } from "console";
export const FileServer = async (req, res) => {
    try {
        // const __filename = fileURLToPath(import.meta.url);
        // const __dirname = dirname(__filename);
        if (!req.file)
            return res.status(400).json("Chưa có file tải lên");
...
export const getFileGoogle = async (req, res) => {
    try {
        const { id } = req.body;
        const auth = await authorize();
...
export const deleteFileGoogle = async (req, res) => {
    try {
        const { id } = req.params;
        const auth = await authorize();
...
export const createFolderFileServer = async (req, res) => {
    try {
        const auth = await authorize();
...
export const uploadFileImageBase64 = async (req, res) => {
    try {
        const { img } = req.body;
        const auth = await authorize();
...
// upload v2
export const getListBucket = async (_, res) => {
    try {
...
export const createBucket_v2 = async (req, res) => {
    try {
...
export const uploadMultiImage_v2 = async (req, res) => {
    try {
        const { files } = req;
        if (!files)
            return res.status(400).json("Chưa có file tải lên");
        ...
        for (const file of files) {
            const img = await sharp(file.path).metadata();
            ...
            switch (img.format) {
                case "png":
                    await sharp(file.path)
                        .png({ quality: 50 })
                        .withMetadata()
                        .toFile(path.join("uploads", `${file.filename}${path.extname(file.originalname)}`)).then(() => {
            ...
            const result = await uploadImageToBucket(imageBucket, fileName, originFileName, mimeType, true);
...
export const uploadFile_v2 = async (req, res) => {
    try {
        const { file } = req;
        if (!file)
            return res.status(400).json("Chưa có file tải lên");
...
export const uploadMultiFile_v2 = async (req, res) => {
    try {
        const { files } = req;
        let listFile = [];
        if (!files)
...
export const uploadMultiFile_congdoan = async (req, res) => {
    try {
        const { files } = req;
        let listFile = [];
        if (!files)
...
//   }
// };
export const getListFile_v2 = async (req, res) => {
    try {
...
export const viewFile_v2 = async (req, res) => {
    try {
