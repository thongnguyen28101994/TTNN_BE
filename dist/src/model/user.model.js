...
import { hashPassword, hashPasswordWithSalt, } from "../satl_password/hashPassword.js";
import { User_test } from "../entities/user_test.js";
export const UserApi = {
    login: async (username, password, donviId) => {
        try {
            //const userRepository = DangKyThi_TTNN_dataSource.getRepository(User);
            const data = await DangKyThi_TTNN_dataSource.query(`select * from [user] where userName=@0 and donviId=@1 and password=@2`, [username, donviId, password]);
    ...
    getDmRole: async () => {
        const data = await DangKyThi_TTNN_dataSource.query("Select * from dm_role");
        return data;
    },
    ...
    //   }
    // },
    changePassword: async (donviId, userId, oldPassword, newPassword) => {
        try {
            // const userRepository = DangKyThi_TTNN_dataSource.getRepository(User);
            // const user = await userRepository.findOne({
            //   where: { donviId: donviId },
            // });
            const user = await DangKyThi_TTNN_dataSource.query(`SELECT * FROM dbo.[user] WHERE donviId = @0 AND userId = @1`, [donviId, userId]);
            ...
            // await userRepository.save(user);
            await DangKyThi_TTNN_dataSource.query(`UPDATE dbo.[user] SET password = @0 WHERE donviId = @1 AND userId = @2`, [newPassword, donviId, userId]);
            console.log("Password changed successfully");
            return true; // Đổi mật khẩu thành công
        }
        catch (err) {
    ...
    updatePasswordWithSalt: async (userId_test) => {
        try {
            ...
            const finalHashedPassword = hashPasswordWithSalt(hashedPassword, salt, 16);
            // Cập nhật cơ sở dữ liệu
    ...
    postUserBep: async (donviId, name, userName) => {
        try {
            const tenTruongResult = await DangKyThi_TTNN_dataSource.query(`SELECT TOP 1 
          CASE 
          ...
          END AS tenTruong
        FROM dbo.[user] 
        WHERE roleId = 3 AND donviId = @0`, [donviId]);
            const donviIdResult = await DangKyThi_TTNN_dataSource.query(`SELECT TOP 1 donviId 
        FROM dbo.[user] 
        WHERE roleId = 3 AND donviId = @0`, [donviId]);
            const tenTruong = tenTruongResult[0]?.tenTruong || "";
            const donviIdValue = donviIdResult[0]?.donviId || "";
            const data = await DangKyThi_TTNN_dataSource.query(`INSERT INTO dbo.[user] (fullName, roleId, userName, [password], donviId, created_date, roleMenu) 
    ...
    getUserBepAn: async (donviId) => {
        try {
            const data = await DangKyThi_TTNN_dataSource.query(`SELECT fullName, username, password 
        FROM dbo.[user] 
        WHERE donviId = @0 AND roleId = 5`, [donviId]);
            return data;
        }
        catch (err) {
