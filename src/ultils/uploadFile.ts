...
export const createFolder = async (auth: any) => {
...
export const getFile = async (auth: any, fileId: string) => {
  const drive = google.drive({ version: "v3", auth });
...
export const deleteFile = async (auth: any, fileId: string) => {
  const drive = google.drive({ version: "v3", auth });
...
export async function uploadFile(authClient: any, sourceFile: any,extension?: string) {
...
export async function uploadFileImageWithBase64(authClient: any, sourceFile: any) {
  return new Promise((resolve, rejected) => {
    const drive = google.drive({ version: "v3", auth: authClient });
